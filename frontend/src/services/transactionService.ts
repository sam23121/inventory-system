...
export const transactionService = {
  ...
  approve: (id: number) => 
    api.post<Transaction>(`/transactions/${id}/approve`),
  reject: (id: number) => 
    api.post<Transaction>(`/transactions/${id}/reject`),
};

export const transactionTypeService = {
  getAll: () => api.get<TransactionType[]>('/transactions/types/'),
  getById: (id: number) => api.get<TransactionType>(`/transactions/types/${id}`),
  create: (data: Omit<TransactionType, 'id'>) => 
    api.post<TransactionType>('/transactions/types', data),
  update: (id: number, data: Partial<TransactionType>) => 
    api.put<TransactionType>(`/transactions/types/${id}`, data),
