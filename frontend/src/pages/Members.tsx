...
import { ReligiousDocument } from '../types/document'; // Ensure you have this type defined
import { memberDocumentService } from '../services/documentService'; // Assume these functions are defined to interact with your backend
import { useTranslation } from 'react-i18next';

const Members: React.FC = () => {
  const [formData, setFormData] = useState<ReligiousDocument>({
    serial_number: '',
    english_name: '',
    english_father_name: '',
    english_mother_name: '',
  ...
  const [religiousDocuments, setReligiousDocuments] = useState<ReligiousDocument[]>([]);
  const [message, setMessage] = useState<{ text: string; type: 'success' | 'error' } | null>(null);
  const { t } = useTranslation();
  useEffect(() => {
    const loadItemTypes = async () => {
      try {
        const types = await memberDocumentService.getAll();
        setReligiousDocuments(types.data);
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validate()) {
      try {
        ...
        await memberDocumentService.create(formData);
