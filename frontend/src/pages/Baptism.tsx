...
import { BaptismDocument } from '../types/document'; // Ensure you have this type defined
import { baptismDocumentService } from '../services/documentService'; // Assume these functions are defined to interact with your backend
import { useTranslation } from 'react-i18next';

const Members: React.FC = () => {
  const [formData, setFormData] = useState<BaptismDocument>({
    serial_number: '',
    english_name: '',
    english_father_name: '',
    english_mother_name: '',
  ...
  const [baptismDocuments, setBaptismDocuments] = useState<BaptismDocument[]>([]);
  ...
  useEffect(() => {
    const loadItemTypes = async () => {
      try {
        const types = await baptismDocumentService.getAll();
        setBaptismDocuments(types.data);
      } catch (error) {
  ...
  const handleChange = (
    ...
    setFormData((prev) => {
      let newValue;
      if (name === 'date_of_birth' || name === 'baptism_date') {
  ...
  const validate = () => {
    ...
    if (!formData.baptism_date) newErrors.baptism_date = 'Baptism date is required';
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validate()) {
      try {
        ...
        await baptismDocumentService.create(formData);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-wrap gap-4" style={{ maxWidth: '100%' }}>
          <div className="flex flex-wrap w-full gap-4">
            ...
            <div className="w-full md:w-1/4 min-w-[200px]">
              <FormField
                label={t('baptism.baptism_date')}
                name="baptism_date"
                type="datetime-local"
                value={formData.baptism_date?.toISOString().slice(0, 16)}
                onChange={handleChange}
            ...
            <div className="w-full md:w-1/4 min-w-[200px]">
              <FormField
                label={t('baptism.english_god_parent_name')}
                name="english_god_parent_name"
                value={formData.english_god_parent_name}
                onChange={handleChange}
              />
            </div>
